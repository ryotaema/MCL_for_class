...
実際のセンサ観測値 $z_{obs}$ と、各パーティクルの位置から予測される観測値 $z_{pred}$ との差（誤差）に基づき、そのパーティクルの尤度（重み $w$）を算出します。誤差分布には多変量ガウス分布を仮定しています。

$$
w \propto \exp\left( - \frac{(r_{obs} - r_{pred})^2}{2\sigma_r^2} - \frac{(\phi_{obs} - \phi_{pred})^2}{2\sigma_\phi^2} \right)
$$
